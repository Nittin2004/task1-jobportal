...
import { sendOtp, registerCompany } from '../services/api';
...
const CompanyRegister = () => {
  ...
  const [form, setForm] = useState({ name: '', email: '', password: '', phone: '', industry: '', location: '', otp: '' });
  ...
  const handleSendOtp = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await sendOtp({ email: form.email.trim(), role: 'company' });
      if (res.data.devOtp) {
        setDevInfoOtp(res.data.devOtp);
        console.log('--- Dev OTP Code ---', res.data.devOtp);
      }
      toast.success('Verification OTP sent to company email!');
      setStep(2);
    } catch (err) {
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      toast.success('New OTP sent to company email!', { id: toastId });
    } catch (err) {
  ...
  const handleSubmitRegistration = async (e) => {
    ...
    try {
      ...
      toast.success('Company registered & verified successfully!');
  ...
  return (
    <div className="auth-page">
      <div className="auth-card">
        ...
        <p>{step === 1 ? 'Post jobs and find the best candidates' : `Verify email sent to ${form.email}`}</p>
        ...
        {step === 1 ? (
          <>
            ...
            <GoogleAuthButton role="company" text="Register Company with Google" />

            <div className="auth-footer" style={{ marginTop: '1.25rem' }}>
              Already have an account? <Link to="/login">Login</Link>
            </div>
            <div className="auth-footer mt-1">
              Are you a candidate? <Link to="/register/candidate">Register as Candidate</Link>
            </div>
          </>
        ) : (
